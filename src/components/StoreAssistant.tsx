...
import { 
  ...
  Ticket, Layout, CreditCard, ShoppingBag, Landmark, Clipboard
} from 'lucide-react';
    ...
    <>
      ...
      {!isOpen && (
        <button
          ...
          className="fixed bottom-6 right-6 z-50 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white p-4 rounded-full shadow-[0_8px_30px_rgba(37,99,235,0.4)] flex items-center justify-center transition-all duration-300 hover:scale-105 group border border-blue-500/20 cursor-pointer"
        >
          <Bot className="w-6 h-6 group-hover:rotate-12 transition-transform" />
          <span className="max-w-0 overflow-hidden group-hover:max-w-xs transition-all duration-500 ease-out font-sans font-black text-[10px] uppercase tracking-widest pl-0 group-hover:pl-2 text-white block whitespace-nowrap">
            Store Assistant
          </span>
        </button>
      )}
      ...
      {isOpen && (
        ...
        >
          {/* Header */}
          <div className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white p-4 flex items-center justify-between shrink-0 border-b border-blue-500/10">
            <div className="flex items-center gap-2.5">
              <div className="p-1.5 bg-white/10 rounded-xl">
                <Bot className="w-5 h-5 text-white" />
              </div>
              <div className="text-left">
                <h3 className="font-extrabold text-xs uppercase tracking-widest text-white">Store Assistant</h3>
                <span className="text-[9px] text-blue-200 flex items-center gap-1 mt-0.5 font-medium">
                  <span className="w-1.5 h-1.5 bg-emerald-400 rounded-full animate-pulse" /> AI Support Agent
                </span>
              </div>
            </div>
                      ...
                      <div className="space-y-1.5 pt-2 text-left">
                        ...
                        <div className="flex flex-col gap-1">
                          {[
                            ...
                            { text: 'How to create coupons?', icon: Ticket },
                            { text: 'How to manage orders?', icon: ShoppingBag },
                            { text: 'How to customize website?', icon: Layout },
                            { text: 'How to connect payment gateway?', icon: CreditCard }
                          ].map((q, idx) => (
                            <button
                              key={idx}
                      ...
                      <>
                        ...
                        <form onSubmit={handleSubmitTicket} className="space-y-3 bg-slate-900 border border-slate-800 p-4 rounded-2xl shadow-sm">
                          ...
                          <button
                            ...
                            className="w-full py-2.5 bg-blue-600 hover:bg-blue-500 text-white font-bold uppercase tracking-widest text-[9px] rounded-xl flex items-center justify-center gap-1 shadow-md shadow-blue-500/10 transition-all cursor-pointer disabled:opacity-50"
                          >
              ...
              {activeTab === 'chat' && (
                <div className="p-3 border-t border-slate-800 bg-slate-900 flex items-center gap-2 shrink-0">
                  ...
                  <button
                    ...
                    disabled={!input.trim()}
                    className="p-2.5 bg-blue-600 text-white rounded-xl hover:bg-blue-500 transition-colors shadow-sm disabled:opacity-40 cursor-pointer"
                  >
