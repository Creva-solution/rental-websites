...
import { Plug, ExternalLink, Settings, Shield, ToggleLeft, ToggleRight, Check } from 'lucide-react';

export default function IntegrationsPage() {
  const [integrations, setIntegrations] = useState([
    { id: 'razorpay', name: 'Razorpay PG', category: 'Payments', desc: 'Secure online card & netbanking gateway integration.', connected: true, logo: '💳' },
    { id: 'shiprocket', name: 'Shiprocket logistics', category: 'Shipping', desc: 'Sync orders, print labels, and coordinate automatic courier dispatches.', connected: true, logo: '📦' },
    { id: 'delhivery', name: 'Delhivery Shipping', category: 'Shipping', desc: 'Fast local express shipping API with automated status webhooks.', connected: false, logo: '🚚' },
    { id: 'whatsapp_api', name: 'Creva WhatsApp Bot', category: 'Automations', desc: 'Real-time automatic order confirmation & UPI screenshot verification bot.', connected: true, logo: '💬' },
    { id: 'ga4', name: 'Google Analytics 4', category: 'Analytics', desc: 'Track customer funnel dropoffs and product checkout conversion rates.', connected: false, logo: '📈' }
  ]);
          ...
          <div key={idx} className="bg-card border rounded-2xl p-5 shadow-sm hover:border-[#3C77C3]/20 flex flex-col justify-between space-y-5 transition-all text-left">
            <div className="space-y-3">
              <div className="flex justify-between items-start">
                <span className="text-3xl shrink-0 bg-muted w-10 h-10 rounded-xl flex items-center justify-center border shadow-inner">
                  {integration.logo}
                </span>
